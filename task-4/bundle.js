	...
	var app = new _vue2.default({
		...
		created: function created() {
			...
			window.onbeforeunload = function () {
				...
				window.localStorage.setItem('mynewTodo', _this.newTodo);
			};
			var oldDataString = window.localStorage.getItem('myTodos');
			var newTodoData = window.localStorage.getItem('mynewTodo');
		...
		methods: {
			addTodo: function addTodo() {
				...
				var d = new Date();
				var date = d.getFullYear() + '年' + (d.getMonth() + 1) + '月' + d.getDate() + '日 ' + d.getHours() + ':' + changeTimeStyle(d.getMinutes()) + ':' + changeTimeStyle(d.getSeconds());
			...
			login: function login() {
				...
				_leancloudStorage2.default.User.logIn(this.formData.username, this.formData.password).then(function (loginedUser) {
					...
					_this3.user = loginedUser._serverData.username;
				}, function (error) {
					alert("登录失败");
				});
			},
